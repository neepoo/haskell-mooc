...
takeFinal :: Int -> [a] -> [a]
-- takeFinal n xs = todo
-- takeFinal n xs = let len = length xs in if len < n then xs else drop (len - n) xs

takeFinal n xs = case len < n of
  True -> xs
  False -> drop (len - n) xs
  where
    len = length xs
...
substring i j s = substringHelper i j s ""

substringHelper :: Int -> Int -> String -> String -> String
-- substringHelper i j s res = case i < j of  True -> substringHelper (i+1) j s (res ++ [s !! i])
--                                            False    -> res
substringHelper i j s res =
  if i < j
    then substringHelper (i + 1) j s (res ++ [s !! i])
    else res
...
isPalindrome str
  | length str > 1 = (head str == last str) && isPalindrome (tail (init str))
  | otherwise = True
