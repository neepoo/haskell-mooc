...
indexDefault (im : ims) 0 _ = im -- ok base case
indexDefault _ (-1) def = def -- invalid index
indexDefault [] _ def = def -- index too large
indexDefault (im : ims) i def = indexDefault ims (i -1) def
...
-- sumsOf xs = todo
sumsOf [] = []
sumsOf (cur : xs) = go cur xs
  where
    go a [] = [a]
    go cur (y : ys) = cur : go (cur + y) ys
...
mymaximum bigger initial (x : xs) =
  if bigger x initial
    ...
    else mymaximum bigger initial xs -- update initial as final result
...
map2 f _ [] = []
map2 f [] _ = []
map2 f (x : xs) (y : yx) = f x y : map2 f xs yx
